...
class TestAdminRegisters:
    def test_register_add_user_success(self, register_user_by_admin):
        try:
    ...
    def test_register_user_duplicate_email(self, login_admin, register_user_by_admin):
        try:
            response = register_user_by_admin
            assert response.status_code == status.HTTP_200_OK
            token = login_admin.json()["access_token"]
    ...
    def test_register_password_validation_errors(
        self, login_admin, password, output_detail
    ):
        try:
            token = login_admin.json()["access_token"]
