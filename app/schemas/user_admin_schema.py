    ...
    def validate_password(cls, password: str) -> str:
        if len(password) < 8:
            raise HTTPException(
                ...
                detail=password_have_atleast_8_characters,
            )
        if not re.search(r"[A-Z]", password):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=password_have_atleast_one_uppercase_letter,
            )
        if not re.search(r"[a-z]", password):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=password_have_atleast_one_lowercase_letter,
            )
        if not re.search(r"\d", password):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=password_have_atleast_one_digit,
            )
        if not re.search(r"[@$!%*?&#]", password):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=password_have_atleast_one_special_character,
            )
        return password
