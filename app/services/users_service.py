from fastapi import Depends, HTTPException, status, BackgroundTasks, Request, Form, Response
...
class UserService:
    ...
    def forgot_password(
        ...
        reset_token = create_access_token(data=token_data, expires_delta=timedelta(minutes=settings.email_expire_minutes)) 
    ...
    def logout(self, current_user: Annotated[User, Depends(get_current_user)],
        ...
        try:
            ...
            data_refresh = jwt.decode(refresh_token, settings.secret_key, algorithms=[settings.algorithm])
            if data_refresh.get("type") != "refresh":
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Not a valid refresh token!")
            
            data_access = jwt.decode(access_token, settings.secret_key, algorithms=[settings.algorithm])
            if data_access.get("type") != "access":
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Not a valid access token!")
        except InvalidTokenError:
